...
class ThreeDeeMap
{
	...
	ThreeDeeMap(string _map_name, string _map_tile_sheet, string _map_lightmap, string _map_heightmap, string _skybox_name)
	{
		...
		if(Texture::exists(_map_name))
		{
			...
			Texture::createFromFile(_map_name, _map_name);
		}
		else
			Texture::createFromFile(_map_name, _map_name);
		...
		if(Texture::exists(_map_lightmap))
		{
			...
			Texture::createFromFile(_map_lightmap, _map_lightmap);
		}
		else
			Texture::createFromFile(_map_lightmap, _map_lightmap);
		...
		if(Texture::exists(_map_heightmap))
		{
			...
			Texture::createFromFile(_map_heightmap, _map_heightmap);
		}
		else
			Texture::createFromFile(_map_heightmap, _map_heightmap);
		...
		if(Texture::exists(_map_tile_sheet))
		{
			...
			Texture::createFromFile(_map_tile_sheet, _map_tile_sheet);
		}
		else
			Texture::createFromFile(_map_tile_sheet, _map_tile_sheet);
				...
				for(int x = 0; x < mapWidth; x++)
				{
					...
					if(height_end >= height_start)
					{
						if(height_start > 0)
						{
							...
							Vertexes.push_back(Vertex(y+1,	0,	x,		0,			1,	lightMapImage.get(x*2, 		y*2+1)));
							...
							if(height_start < 25)
							{
								...
								Vertexes.push_back(Vertex(y+1,	height_start,	x+1,	x_UVStep,	1,	lightMapImage.get(x*2+1,	y*2+1)));
							}
						}
						
						//default wall texture
						//if(clr == map_colors::floor || clr == map_colors::green_banner || clr == map_colors::red_banner || clr == map_colors::green_spawn || clr == map_colors::red_spawn)
						//{
						//	//actual floor
						//	Vertexes.push_back(Vertex(y,	0,	x,		0,			0,	lightMapImage.get(x*2, 		y*2)));
						//	Vertexes.push_back(Vertex(y,	0,	x+1,	x_UVStep,	0,	lightMapImage.get(x*2+1, 	y*2)));
					...
					/*{
						...
						if(height_start < 26)
						{
							...
							bool right = 	(x+1 < mapWidth) && 	(heightMapImage.get(x+1, y).getRed()/10+1 < 26) && (heightMapImage.get(x+1, y).getRed()/10+1 > height_start);
							bool left = 	(x-1 >= 0) && 			(heightMapImage.get(x-1, y).getRed()/10+1 < 26) && (heightMapImage.get(x-1, y).getRed()/10+1 > height_start);
							bool down = 	(y+1 < mapHeight) && 	(heightMapImage.get(x, y+1).getRed()/10+1 < 26) && (heightMapImage.get(x, y+1).getRed()/10+1 > height_start);
							bool up = 		(y-1 >= 0) && 			(heightMapImage.get(x, y-1).getRed()/10+1 < 26) && (heightMapImage.get(x, y-1).getRed()/10+1 > height_start);
							
							if(right)
							{
								uint heightDiff = heightMapImage.get(x+1, y).getRed()/10+1 - height_start;
								for(int i = height_start; i < height_start+heightDiff; i++)
								{
									Vertexes.push_back(Vertex(y+1,	i+1,	x+1,	x_UVStep*3,0,	lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y,	i+1,	x+1,	x_UVStep*4,0,	lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y,	i,		x+1,	x_UVStep*4,1,	lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y+1,	i,		x+1,	x_UVStep*3,1,	lightMapImage.get(x*3+2, y*3+2)));
								}
							}
							...
							if(left)
							{
								...
								for(int i = height_start; i < height_start+heightDiff; i++)
								{
									Vertexes.push_back(Vertex(y,	i+1,	x,	x_UVStep*3,0,	lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y+1,	i+1,	x,	x_UVStep*4,0,	lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	i,		x,	x_UVStep*4,1,	lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y,	i,		x,	x_UVStep*3,1,	lightMapImage.get(x*3, y*3)));
								}
							}
							
							if(down)
							{
								uint heightDiff = heightMapImage.get(x, y+1).getRed()/10+1 - height_start;
								for(int i = height_start; i < height_start+heightDiff; i++)
								{
									Vertexes.push_back(Vertex(y+1,	i+1,	x,		x_UVStep*3,0,	lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	i+1,	x+1,	x_UVStep*4,0,	lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	i,		x+1,	x_UVStep*4,1,	lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	i,		x,		x_UVStep*3,1,	lightMapImage.get(x*3, y*3+2)));
								}
							}
							...
							if(up)
							{
								...
								for(int i = height_start; i < height_start+heightDiff; i++)
								{
									Vertexes.push_back(Vertex(y,	i+1,	x+1,	x_UVStep*3,0,	lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y,	i+1,	x,		x_UVStep*4,0,	lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y,	i,		x,		x_UVStep*4,1,	lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y,	i,		x+1,	x_UVStep*3,1,	lightMapImage.get(x*3+2, y*3)));
								}
							}
						}
					}*/
					...
					/*switch(clr)
					{
						...
						case map_colors::red_spawn:
						{
							...
							// Floor
							Vertexes.push_back(Vertex(y,	0,	x,		0,0,		lightMapImage.get(x*3, y*3)));
							Vertexes.push_back(Vertex(y,	0,	x+1,	1,0,		lightMapImage.get(x*3+2, y*3)));
							Vertexes.push_back(Vertex(y+1,	0,	x+1,	1,yUVStep,	lightMapImage.get(x*3+2, y*3+2)));
							Vertexes.push_back(Vertex(y+1,	0,	x,		0,yUVStep,	lightMapImage.get(x*3, y*3+2)));
							
							uint height = heightMapImage.get(x, y).getBlue()/20+1;
							
							if(height < 12)
							{
								// Ceiling
								Vertexes.push_back(Vertex(y,	height,	x+1,	0,0.9,			lightMapImage.get(x*3+2, y*3)));
								Vertexes.push_back(Vertex(y,	height,	x,		1,0.9,			lightMapImage.get(x*3, y*3)));
								Vertexes.push_back(Vertex(y+1,	height,	x,		1,0.9+yUVStep,	lightMapImage.get(x*3, y*3+2)));
								Vertexes.push_back(Vertex(y+1,	height,	x+1,	0,0.9+yUVStep,	lightMapImage.get(x*3+2, y*3+2)));
								
								// Ceiling merging -------------------------
								bool right = 	(x+1 < mapWidth) && 	(heightMapImage.get(x+1, y).getBlue()/20+1 < 12) && (heightMapImage.get(x+1, y).getBlue()/20+1 > height);
								bool left = 	(x-1 >= 0) && 			(heightMapImage.get(x-1, y).getBlue()/20+1 < 12) && (heightMapImage.get(x-1, y).getBlue()/20+1 > height);
								bool down = 	(y+1 < mapHeight) && 	(heightMapImage.get(x, y+1).getBlue()/20+1 < 12) && (heightMapImage.get(x, y+1).getBlue()/20+1 > height);
								bool up = 		(y-1 >= 0) && 			(heightMapImage.get(x, y-1).getBlue()/20+1 < 12) && (heightMapImage.get(x, y-1).getBlue()/20+1 > height);
								
								if(right)
								{
									uint heightDiff = height - heightMapImage.get(x+1, y).getBlue()/20+1;
									for(int i = height; i < height+heightDiff; i++)
									{
										Vertexes.push_back(Vertex(y+1,	i+1,	x+1,	0,0.9,			lightMapImage.get(x*3+2, y*3+2)));
										Vertexes.push_back(Vertex(y,	i+1,	x+1,	1,0.9,			lightMapImage.get(x*3+2, y*3)));
										Vertexes.push_back(Vertex(y,	i,		x+1,	1,0.9+yUVStep,	lightMapImage.get(x*3+2, y*3)));
										Vertexes.push_back(Vertex(y+1,	i,		x+1,	0,0.9+yUVStep,	lightMapImage.get(x*3+2, y*3+2)));
									}
								}
								...
								if(left)
								{
									...
									for(int i = height; i < height+heightDiff; i++)
									{
										Vertexes.push_back(Vertex(y,	i+1,	x,	0,0.9,			lightMapImage.get(x*3, y*3)));
										Vertexes.push_back(Vertex(y+1,	i+1,	x,	1,0.9,			lightMapImage.get(x*3, y*3+2)));
										Vertexes.push_back(Vertex(y+1,	i,		x,	1,0.9+yUVStep,	lightMapImage.get(x*3, y*3+2)));
										Vertexes.push_back(Vertex(y,	i,		x,	0,0.9+yUVStep,	lightMapImage.get(x*3, y*3)));
									}
								}
								
								if(down)
								{
									uint heightDiff = height - heightMapImage.get(x, y+1).getBlue()/20+1;
									for(int i = height; i < height+heightDiff; i++)
									{
										Vertexes.push_back(Vertex(y+1,	i+1,	x,		0,0.9,			lightMapImage.get(x*3, y*3+2)));
										Vertexes.push_back(Vertex(y+1,	i+1,	x+1,	1,0.9,			lightMapImage.get(x*3+2, y*3+2)));
										Vertexes.push_back(Vertex(y+1,	i,		x+1,	1,0.9+yUVStep,	lightMapImage.get(x*3+2, y*3+2)));
										Vertexes.push_back(Vertex(y+1,	i,		x,		0,0.9+yUVStep,	lightMapImage.get(x*3, y*3+2)));
									}
								}
								...
								if(up)
								{
									...
									for(int i = height; i < height+heightDiff; i++)
									{
										Vertexes.push_back(Vertex(y,	i+1,	x+1,	0,0.9,			lightMapImage.get(x*3+2, y*3)));
										Vertexes.push_back(Vertex(y,	i+1,	x,		1,0.9,			lightMapImage.get(x*3, y*3)));
										Vertexes.push_back(Vertex(y,	i,		x,		1,0.9+yUVStep,	lightMapImage.get(x*3, y*3)));
										Vertexes.push_back(Vertex(y,	i,		x+1,	0,0.9+yUVStep,	lightMapImage.get(x*3+2, y*3)));
									}
								}
								//------------------------------------------
							}
						...
						{
							bool right = (	x+1 < mapWidth && (
											mapImage.get(x+1, y).color == map_colors::floor ||
											mapImage.get(x+1, y).color == map_colors::green_banner ||
											mapImage.get(x+1, y).color == map_colors::red_banner));
							...
							bool down = (	y+1 < mapHeight && (
											mapImage.get(x, y+1).color == map_colors::floor ||
											mapImage.get(x, y+1).color == map_colors::green_banner ||
											mapImage.get(x, y+1).color == map_colors::red_banner));
							...
							Vec2f uv = UVfromColor(clr);
							
							uint highestHeight = 0;
							for(int h = 0; h < 4; h++)
							{
								Vec2f tempPos = positionsFour[h];
								if(x+positionsFour[h].x >= mapWidth || x+positionsFour[h].x < 0 || y+positionsFour[h].y >= mapHeight || y+positionsFour[h].y < 0) continue;
								
								if(highestHeight < heightMapImage.get(x+positionsFour[h].x, y+positionsFour[h].y).getBlue()/20+1 && heightMapImage.get(x+positionsFour[h].x, y+positionsFour[h].y).getBlue()/20+1 < 12)
									highestHeight = heightMapImage.get(x+positionsFour[h].x, y+positionsFour[h].y).getBlue()/20+1;
							}
							
							highestHeight = highestHeight == 0 ? 12 : highestHeight;
							
							if(right)
							{
								uint roof = heightMapImage.get(x+1, y).getBlue()/20+1;
								uint height = roof < 12 ? roof : highestHeight;
								for(int i = 0; i < height; i++)
								{
									Vertexes.push_back(Vertex(y+1,	i+1,	x+1,	0,uv.y,			lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y,	i+1,	x+1,	1,uv.y,			lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y,	i,		x+1,	1,uv.y+yUVStep,	lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y+1,	i,		x+1,	0,uv.y+yUVStep,	lightMapImage.get(x*3+2, y*3+2)));
								}
								
								//	Decals
								if(mapImage.get(x+1, y).color == map_colors::green_banner || mapImage.get(x+1, y).color == map_colors::red_banner)
								{
									Vec2f decalsUV = UVfromColor(mapImage.get(x+1, y).color);
									Vertexes.push_back(Vertex(y+1,	1,	x+1.02f,	0,decalsUV.y,			lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y,	1,	x+1.02f,	1,decalsUV.y,			lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y,	0,	x+1.02f,	1,decalsUV.y+yUVStep,	lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y+1,	0,	x+1.02f,	0,decalsUV.y+yUVStep,	lightMapImage.get(x*3+2, y*3+2)));
								}
							}
							...
							if(left)
							{
								...
								for(int i = 0; i < height; i++)
								{
									Vertexes.push_back(Vertex(y,	i+1,	x,	0,uv.y,			lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y+1,	i+1,	x,	1,uv.y,			lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	i,		x,	1,uv.y+yUVStep,	lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y,	i,		x,	0,uv.y+yUVStep,	lightMapImage.get(x*3, y*3)));
								}
								
								if(mapImage.get(x-1, y).color == map_colors::green_banner || mapImage.get(x-1, y).color == map_colors::red_banner)
								{
									Vec2f decalsUV = UVfromColor(mapImage.get(x-1, y).color);
									Vertexes.push_back(Vertex(y,	1,	x-0.02f,	0,decalsUV.y,			lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y+1,	1,	x-0.02f,	1,decalsUV.y,			lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	0,	x-0.02f,	1,decalsUV.y+yUVStep,	lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y,	0,	x-0.02f,	0,decalsUV.y+yUVStep,	lightMapImage.get(x*3, y*3)));
								}
							}
							
							if(down)
							{
								uint roof = heightMapImage.get(x, y+1).getBlue()/20+1;
								uint height = roof < 12 ? roof : highestHeight;
								for(int i = 0; i < height; i++)
								{
									Vertexes.push_back(Vertex(y+1,	i+1,	x,		0,uv.y,			lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	i+1,	x+1,	1,uv.y,			lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	i,		x+1,	1,uv.y+yUVStep,	lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y+1,	i,		x,		0,uv.y+yUVStep,	lightMapImage.get(x*3, y*3+2)));
								}
								
								if(mapImage.get(x, y+1).color == map_colors::green_banner || mapImage.get(x, y+1).color == map_colors::red_banner)
								{
									Vec2f decalsUV = UVfromColor(mapImage.get(x, y+1).color);
									Vertexes.push_back(Vertex(y+1.02f,	1,	x,		0,decalsUV.y,			lightMapImage.get(x*3, y*3+2)));
									Vertexes.push_back(Vertex(y+1.02f,	1,	x+1,	1,decalsUV.y,			lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y+1.02f,	0,	x+1,	1,decalsUV.y+yUVStep,	lightMapImage.get(x*3+2, y*3+2)));
									Vertexes.push_back(Vertex(y+1.02f,	0,	x,		0,decalsUV.y+yUVStep,	lightMapImage.get(x*3, y*3+2)));
								}
							}
							...
							if(up)
							{
								...
								for(int i = 0; i < height; i++)
								{
									Vertexes.push_back(Vertex(y,	i+1,	x+1,	0,uv.y,			lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y,	i+1,	x,		1,uv.y,			lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y,	i,		x,		1,uv.y+yUVStep,	lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y,	i,		x+1,	0,uv.y+yUVStep,	lightMapImage.get(x*3+2, y*3)));
								}
								
								if(mapImage.get(x, y-1).color == map_colors::green_banner || mapImage.get(x, y-1).color == map_colors::red_banner)
								{
									Vec2f decalsUV = UVfromColor(mapImage.get(x, y-1).color);
									Vertexes.push_back(Vertex(y-0.02f,	1,	x+1,	0,decalsUV.y,			lightMapImage.get(x*3+2, y*3)));
									Vertexes.push_back(Vertex(y-0.02f,	1,	x,		1,decalsUV.y,			lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y-0.02f,	0,	x,		1,decalsUV.y+yUVStep,	lightMapImage.get(x*3, y*3)));
									Vertexes.push_back(Vertex(y-0.02f,	0,	x+1,	0,decalsUV.y+yUVStep,	lightMapImage.get(x*3+2, y*3)));
								}
							}
