...
*/
/*
/rcon CBlob@[]b;getBlobs(b);for(int i=0;i<b.length();i++){CBlob@n=b[i];if(n!is null){CPlayer@p=n.getPlayer();if(p!is null)n.server_RemoveFromInventories();}}

/rcon u32 id=getPlayerByUsername('LaikaTheSpaceDog').getNetworkID();CBitStream p;p.write_u16(id);p.write_u16(id);p.write_string('i fuck animals');getRules().SendCommand(getRules().getCommandID('vote: kick'), p);
*/